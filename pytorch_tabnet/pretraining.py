...
from pytorch_tabnet.pretraining_utils import (
    create_dataloaders,
    validate_eval_set,
    create_dataloadersStock,
    validate_eval_set_stock,
)
...
class TabNetPretrainerStock(TabModel):
    def __post_init__(self):
        super().__post_init__()
        self._task = "unsupervised"
        self._default_loss = UnsupervisedLoss
    ...
    def fit(
        ...
        # Validate and reformat eval set depending on training data
        eval_names = validate_eval_set_stock(eval_set, eval_name, X_train)
        train_dataloader, valid_dataloaders = self._construct_loaders(X_train, eval_set)
    ...
    def _construct_loaders(self, X_train, eval_set):
        ...
        """
        train_dataloader, valid_dataloaders = create_dataloadersStock(
            X_train,
            eval_set,
            self.updated_weights,
            self.batch_size,
    ...
    def predict(self, X):
        ...
        if scipy.sparse.issparse(X):
            raise TypeError("Sparse matrix not supported in stock pretraining")
        else:
