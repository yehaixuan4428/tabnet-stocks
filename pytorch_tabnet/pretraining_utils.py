...
def create_dataloadersStock(
    ...
    if scipy.sparse.issparse(X_train):
        raise TypeError("Sparse matrix not supported for stock data")
    else:
        train_dataloader = StockPredictDataLoaderCS(
            StockPredictDatasetCS(X_train),
            sampler=sampler,
            shuffle=need_shuffle,
            num_workers=num_workers,
            drop_last=drop_last,
...
def validate_eval_set_stock(eval_set, eval_name, X_train):
    """Check if the shapes of eval_set are compatible with X_train.
